...
int jobs_list_add(pid_t pid, char estado, char *cmd) {
    // N_JOBS-1 porque el primer elemento está reservado para foreground
...
int internal_cd(char **args) {
    ...
    char *nwd = NULL; // almacenar nuevo directorio
    if (argc == 0) { 
        nwd = getenv("HOME");
    } else if (argc == 1) { // asigna nuevo directorio
        nwd = args[1];
    } else {
        // cd avanzado
    }

    // verifica si el directorio es NULL
    if (nwd == NULL) {
...
int internal_export(char **args) {
    char *arg = args[1];
    if (arg == NULL) { // verificar sintaxis 
...
int internal_source(char **args) {
    ...
    while (fgets(line, COMMAND_LINE_SIZE, file)) {
        ...
        fflush(file);
        execute_line(line); // ejecuta la linea 
    }
...
int internal_fg(char **args) {
    ...
    // enviar la señal SIGCONT si el estado es 'D'
    if (job->estado == 'D') {
...
int internal_bg(char **args) {
    ...
    struct info_job *job = &jobs_list[pos];
    // comprobar que el estado es 'E'
    if (job->estado == 'E') {
        ERROR("el proceso %d (%s) ya esta en ejecución", job->pid, job->cmd);
        return -1;
    }
    // si no lo es, cambiar el estado a 'E'
    job->estado = 'E';
    strcat(job->cmd, " &"); // añade '&' al final
    kill(job->pid, SIGCONT);
    DEBUG("internal_fg() -> señal SIGCONT enviada al proceso %d (%s)", job->pid, job->cmd);
...
void ctrlz(int signum) {
    ...
    if (job->pid > 0) {
        ...
        if (strcmp(job->cmd, mi_shell) != 0) {
            ...
            job->estado = 'D';
            int pos = jobs_list_add(job->pid, job->estado, job->cmd); // incorpora el proceso en la lista, al final
...
int parse_args(char **args, char *line) {
    ...
    // itera mientras no encuentra un '#'
    while (token && i < ARGS_SIZE-1 && token[0] != '#') {
        args[i++] = token;
        token = strtok(NULL, ARGS_SEP);
    }
    args[i] = NULL; // pone el ultimo a null
#if DEBUGN6
...
int execute_line(char *line) {
    ...
    // comprueba que sea un comando interno
    if (check_internal(args) > 0) {
        int background = is_background(args); // verificar si comando es background
        pid_t pid = fork(); 
        if (pid < 0) {
